...
// the schema-agnostic sqldump engine over paperless's own tables (from ent
// migrate.Tables). Replaces the legacy per-field paperless.service.v1.BackupService,
...
func NewSqlBackupService(ctx *bootstrap.Context) *SqlBackupService {
	...
	return &SqlBackupService{
		log:    ctx.NewLoggerHelper("paperless/service/sql-backup"),
		engine: sqldump.New(dsn, sqldump.Options{Module: "paperless", Tables: tables}),
	}
}
...
func (s *SqlBackupService) ImportBackup(stream commonV1.BackupService_ImportBackupServer) error {
	...
	if opts == nil {
		return errors.New("paperless: first ImportBackup message must carry options")
	}
	...
	if err != nil {
		...
		return stream.SendAndClose(&commonV1.ImportBackupResponse{Success: false, Module: "paperless", Warnings: []string{err.Error()}})
	}
	out := &commonV1.ImportBackupResponse{Success: true, Module: "paperless", Warnings: res.Warnings}
