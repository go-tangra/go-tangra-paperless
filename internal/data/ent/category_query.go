...
type CategoryQuery struct {
	...
	withPermissions *DocumentPermissionQuery
...
func (_q *CategoryQuery) QueryDocuments() *DocumentQuery {
...
func (_q *CategoryQuery) QueryPermissions() *DocumentPermissionQuery {
	query := (&DocumentPermissionClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *CategoryQuery) WithDocuments(opts ...func(*DocumentQuery)) *CategoryQuery {
	query := (&DocumentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CategoryQuery) WithPermissions(opts ...func(*DocumentPermissionQuery)) *CategoryQuery {
	query := (&DocumentPermissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(category.FieldCreateBy).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := _q.withDocuments; query != nil {
		if err := _q.loadDocuments(ctx, query, nodes,
			func(n *Category) { n.Edges.Documents = []*Document{} },
			func(n *Category, e *Document) { n.Edges.Documents = append(n.Edges.Documents, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPermissions; query != nil {
		if err := _q.loadPermissions(ctx, query, nodes,
			func(n *Category) { n.Edges.Permissions = []*DocumentPermission{} },
			func(n *Category, e *DocumentPermission) { n.Edges.Permissions = append(n.Edges.Permissions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CategoryQuery) loadDocuments(ctx context.Context, query *DocumentQuery, nodes []*Category, init func(*Category), assign func(*Category, *Document)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(document.FieldCategoryID)
	}
	query.Where(predicate.Document(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.DocumentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *CategoryQuery) loadPermissions(ctx context.Context, query *DocumentPermissionQuery, nodes []*Category, init func(*Category), assign func(*Category, *DocumentPermission)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.DocumentPermission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.PermissionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.category_permissions
		if fk == nil {
			return fmt.Errorf(`foreign-key "category_permissions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "category_permissions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *CategoryQuery) ForUpdate(opts ...sql.LockOption) *CategoryQuery {
...
func (_q *CategoryQuery) ForShare(opts ...sql.LockOption) *CategoryQuery {
	if _q.driver.Dialect() == dialect.Postgres {
		_q.Unique(false)
	}
	_q.modifiers = append(_q.modifiers, func(s *sql.Selector) {
