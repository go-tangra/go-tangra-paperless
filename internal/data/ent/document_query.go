...
func (_q *DocumentQuery) QueryCategory() *CategoryQuery {
...
func (_q *DocumentQuery) QueryPermissions() *DocumentPermissionQuery {
...
func (_q *DocumentQuery) WithCategory(opts ...func(*CategoryQuery)) *DocumentQuery {
	query := (&CategoryClient{config: _q.config}).Query()
...
func (_q *DocumentQuery) WithPermissions(opts ...func(*DocumentPermissionQuery)) *DocumentQuery {
	query := (&DocumentPermissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(document.FieldCreateBy).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *DocumentQuery) prepareQuery(ctx context.Context) error {
	...
	if document.Policy == nil {
		return errors.New("ent: uninitialized document.Policy (forgotten import ent/runtime?)")
	}
	if err := document.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *DocumentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Document, error) {
	...
	if query := _q.withCategory; query != nil {
		if err := _q.loadCategory(ctx, query, nodes, nil,
			func(n *Document, e *Category) { n.Edges.Category = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPermissions; query != nil {
		if err := _q.loadPermissions(ctx, query, nodes,
			func(n *Document) { n.Edges.Permissions = []*DocumentPermission{} },
			func(n *Document, e *DocumentPermission) { n.Edges.Permissions = append(n.Edges.Permissions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DocumentQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*Document, init func(*Document), assign func(*Document, *Category)) error {
...
func (_q *DocumentQuery) loadPermissions(ctx context.Context, query *DocumentPermissionQuery, nodes []*Document, init func(*Document), assign func(*Document, *DocumentPermission)) error {
	...
	query.Where(predicate.DocumentPermission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(document.PermissionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.document_permissions
		if fk == nil {
			return fmt.Errorf(`foreign-key "document_permissions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "document_permissions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *DocumentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCategory != nil {
			_spec.Node.AddColumnOnce(document.FieldCategoryID)
		}
	}
