// Package blob provides an object-storage abstraction for the paperless
// service: a small Store interface plus a real S3/MinIO-backed implementation
// and (in fake.go) an in-memory fake for tests and dev.
//
// Real-client path: this file uses the MinIO Go SDK (github.com/minio/minio-go/v7).
// `go get github.com/minio/minio-go/v7@latest` succeeded in this environment, so
// the hand-rolled stdlib net/http + AWS SigV4 fallback (which would have been
// adapted from services/deployer/internal/providers/awsacm/awsacm.go) was not
// needed. Put computes the SHA-256 of the uploaded bytes by teeing the reader
...
//
// Security: AccessKey/SecretKey arrive already-unsealed from the caller (the app
// opens them via the sealed package). This package MUST NEVER log the secret key
...
// Store is the object-storage abstraction used by the paperless service.
